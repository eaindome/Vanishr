...
"""
    ...
    try:
        ...
        logger.error(f"Email task failed for request_id={request_id}: {e}\n{traceback.format_exc()}")
        req.status = "Failed"
    ...
    try:
        ...
        logger.error(f"Failed to update request status in DB for request_id={request_id}: {e}\n{traceback.format_exc()}")
...
async def submit_form_task(request_id: int):
    ...
    try:
        async with async_playwright() as p:
            ...
            await browser.close()
        req.status = "Completed"
        logger.info(f"Form submitted successfully for request_id={request_id}")
    except Exception as e:
        logger.error(f"Form task failed for request_id={request_id}: {e}\n{traceback.format_exc()}")
        req.status = "Failed"
...
def generate_pdf_task(request_id: int):
    ...
    try:
        ...
        req.status = "Completed"
        logger.info(f"PDF generated successfully for request_id={request_id} at {pdf_file}")
    except Exception as e:
        logger.error(f"PDF generation failed for request_id={request_id}: {e}\n{traceback.format_exc()}")
        req.status = "Failed"
    try:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                if broker.opt_out_method == "email":
                    ...
                    session.add(req)
                    await session.commit()
                    logger.error(f"Unknown opt_out_method for request_id={request_id}: {broker.opt_out_method}")
        except Exception as e:
            logger.error(f"Processing failed for request_id={request_id}: {e}\n{traceback.format_exc()}")
